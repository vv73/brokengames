...
package com.example.brokengames.snake;
...
public class SnakeView extends TileView {
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_DOWN) {
			...
			else if (WEST == mDirection)
				mNextDirection = NORTH;

			else if (NORTH == mDirection)
				mNextDirection = EAST;
			else if (EAST == mDirection)
				mNextDirection = SOUTH;
	...
	/*
	 * The original example required a D-Pad present on the device. Since I have
	 * a Samsung Galaxy Mini (GT-S5570) I copied the behaviour from the
	 * onKeyDown() to this onTouchEvent() method. Therefore, you can control the
	 * snake by touching the phone screen.
	 */
	...
	private void updateSnake() {
		...
		for (int appleindex = 0; appleindex < applecount; appleindex++) {
			...
			// check equal coords of the head and an apple
			if (c == newHead) {
				mAppleList.remove(c);
				addRandomApple();
